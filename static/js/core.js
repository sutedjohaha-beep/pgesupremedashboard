...
function doLogin(){
  ...
  if(!found){err.textContent='Username atau password salah.';return;}
  err.textContent='';setCurrent(found);startApp(found);
}
...
function doSelfRegister(){
  ...
  if(code!==ADMIN_CODE){err.textContent='Kode admin tidak valid.';return;}
  const users=getUsers();
  if(users.find(x=>x.username===user)){err.textContent='Username sudah digunakan.';return;}
  users.push({username:user,password:pass,name,role:'pic',dept,wa,email:'',color:AVT[users.length%AVT.length]});
  saveUsers(users);toast('Akun berhasil! Silakan masuk.','g');switchAuth('login');
}
function doLogout(){
  clearInterval(autoInterval);sessionStorage.removeItem('pge_cur');
...
function initAdmin(){
  ...
  if(!u.find(x=>x.username==='admin'))
    u.push({username:'admin',password:'Admin2025!',name:'Admin Audit',role:'admin',dept:'Admin',wa:'',email:'',color:'#4d8ef7'});
  saveUsers(u);
}
initAdmin();
...
const getEffData=()=>{
  ...
  return AUDIT_DATA.map(r=>{const u=upd[r.id];return u?{...r,pct_realisasi:u.pct,catatan_update:u.note,updated_at:u.at,updated_by:u.by,deliverables:u.deliv||r.deliverables}:r;});
};
...
function startApp(user){
  ...
  const pics=[...new Set(AUDIT_DATA.map(r=>r.pic).filter(Boolean))].sort();
...
function saveEditUser(){
  const u=getUsers();const idx=u.findIndex(x=>x.username===selUser);if(idx<0)return;
  u[idx].name=document.getElementById('euName').value.trim()||u[idx].name;
  u[idx].wa=document.getElementById('euWa').value.trim();
  u[idx].email=document.getElementById('euEmail').value.trim();
  u[idx].dept=document.getElementById('euDept').value;
  const np=document.getElementById('euPass').value;if(np.length>=6)u[idx].password=np;
  saveUsers(u);renderUMList();showUserDetail(selUser);closeModal('editUserModal');
  toast('Pengguna diperbarui','g');
}
function delUser(){
  if(!selUser||selUser==='admin')return;
  const u=getUsers().find(x=>x.username===selUser);if(!u)return;
  if(!confirm(`Hapus akun "${u.name}"?`))return;
  saveUsers(getUsers().filter(x=>x.username!==selUser));
  selUser=null;document.getElementById('umEmpty').style.display='flex';document.getElementById('umDetailBody').style.display='none';
...
function doAddUser(){
  ...
  if(!name||!user||!pass){err.textContent='Nama, username, password wajib diisi.';return;}
  const users=getUsers();
  if(users.find(u=>u.username===user)){err.textContent='Username sudah digunakan.';return;}
  users.push({username:user,password:pass,name,role,dept,wa,email,color:AVT[users.length%AVT.length]});
...
function submitUpdate(){
  ...
  const upd=getUpdates();upd[updTarget.id]={pct:updPct,note:document.getElementById('updNote').value.trim(),deliv:document.getElementById('updDeliv').value.trim(),at:TODAY,by:CUR.name};
  saveUpdates(upd);
  // Save pending TL images
  if(window._pendingTlImgs&&window._pendingTlImgs.length){
    const uplKey='tl_imgs_'+updTarget.id;
    const existing=JSON.parse(localStorage.getItem(uplKey)||'[]');
    localStorage.setItem(uplKey,JSON.stringify([...existing,...window._pendingTlImgs]));
    window._pendingTlImgs=[];
    renderUpdTlPreview();
  }
...
// ===== AUTO LOGIN =====
const _s=getCurrent();if(_s){CUR=_s;startApp(_s);
  // After app starts, auto-open update form if URL has ?upd= param
  if(window._autoOpenUpdate){
    setTimeout(()=>{goUpd(window._autoOpenUpdate);window._autoOpenUpdate=null;},500);
  }
}
else document.getElementById('regTab').style.display='';
...
function handleFileInput(event, rid){
  ...
  files.forEach(file=>{
    compressFile(file, uri=>{
      const uplKey='tl_imgs_'+rid;
      const existing=JSON.parse(localStorage.getItem(uplKey)||'[]');
      existing.push(uri);
      localStorage.setItem(uplKey,JSON.stringify(existing));
      // Refresh the TL gallery in detail panel
      const gallery=document.getElementById('tlGallery_'+rid);
      if(gallery){
        const data=getEffData();const r=data.find(x=>x.id===rid);
        if(r){const imgs=getImgs(r);gallery.innerHTML=buildTlGallery(imgs.tl,rid);}
      }
...
function handleDrop(event, rid){
  ...
  const files=Array.from(event.dataTransfer.files).filter(validateFile);
  const fakeEvent={target:{files,value:''}};
  handleFileInput({target:{files,value:''}}, rid);
}
function delTlImg(rid, userIdx){
  if(!confirm('Hapus foto tindak lanjut ini?'))return;
  const uplKey='tl_imgs_'+rid;
  const existing=JSON.parse(localStorage.getItem(uplKey)||'[]');
...
function handleUpdFileInput(event){
  ...
  files.forEach(file=>{
    compressFile(file, uri=>{
      ...
      window._pendingTlImgs.push(uri);
      renderUpdTlPreview();
      toast('Foto siap · klik Simpan untuk menyimpan','g');
    });
  });
  event.target.value='';
}
