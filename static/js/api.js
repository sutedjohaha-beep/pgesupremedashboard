...
function getUpdates() {
  // Return updates map from merged findings
  const m = {};
  _findings.forEach(r => {
    if (r.updated_by) m[r.id] = { pct: r.pct_realisasi, note: r.catatan_update, deliv: r.deliverables, at: r.updated_at, by: r.updated_by };
  });
  return m;
}
...
async function submitUpdate() {
  ...
  if (newFindings) _findings = newFindings;
